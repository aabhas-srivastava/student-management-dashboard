...
export default function StudentsPage() {
  ...
  const filteredStudents = useMemo(() => {
    return students.filter((student) => {
      ...
      if (appliedFilters.course !== "All" && student.course !== appliedFilters.course) return false;
      if (appliedFilters.status !== "All" && student.status !== appliedFilters.status) return false;
  ...
  return (
    <Box>
      ...
      >
        <Typography variant="h5" sx={{ fontWeight: 600, color: "#111111", letterSpacing: -0.4 }}>
          Students
        </Typography>
      ...
      {filteredStudents.length === 0 ? (
        ...
        <StudentTable
          ...
          onRefresh={() => getStudents().then(setStudents)}
        />
      )}
    </Box>
  );
